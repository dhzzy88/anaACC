...
//float ptrange[2] = {2.3,2.5}; //slice 2d hists to this range
...
void fitInit(int itof, int icent, int ich, int ptbin) {
  ...
  if(ich ==0 && runnoEP==0) fitTuning = loadFitParamsNeg(ptbin);
  if(ich ==1 && runnoEP==0) fitTuning = loadFitParamsPos(ptbin);
  if(runnoEP==1) fitTuning = loadFitParamsnoEP();
...
void override() {
  ...
  tuneprotwidth[0] = -0.1;  tuneprotheight[0] = 0.2;  tuneprotmean[0] = 0.05;
  tuneprotwidth[1] = -0.35;  tuneprotmean[1] = -0.035;
  tuneprotheight[3] = 0.1;
  tuneprotwidth[4] = -0.15;
  tuneprotwidth[5] = -0.3;  tuneprotheight[5] = -0.05;

  tunekaonwidth[0] = 0.08;   tunekaonheight[0] = 0.15;
  tunekaonwidth[1] = 0.1;   tunekaonheight[1] = 0.15;
  tunekaonheight[4] = 0.07;
  tunekaonheight[5] = 0.1;
  //tunepionheight[2] = 0.05;
  //tunepionheight[4] = 0.03;
  //tunepionheight[5] = -0.01;
  //tuneprotmean[2] = 0.1;

  //tunepionwidth[2] = 0.99;
...
parstruct unityTuning() {
  ...
  kaonmeanshift = 0;
  protmeanshift = 0;
  fitTuning = setUniformTuning();
  
  
}

parstruct setUniformTuning() {
  for(int i=0; i<6; i++) { 
